...
// Маппинг для красивого отображения срочности
const URGENCY_META = {
...
export default function Dashboard() {
  ...
  }, [filterCategory, filterUrgency, logout, navigate]);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      // Оптимистичное удаление — удаляем из стейта без перезагрузки
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Пользователь: <span className="font-medium text-slate-700">{user?.username}</span>
            </p>
          </div>
          <button
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {posts.map((post) => {
                    const urgencyMeta = URGENCY_META[post.urgency] || {
                      label: post.urgency,
                    ...
                    return (
                      <tr key={post.id} className="hover:bg-slate-50 transition">
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-block px-2 py-1 text-xs font-medium rounded ${urgencyMeta.color}`}
                          >
                            {urgencyMeta.label}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <span
                            className={`inline-block px-2 py-1 text-xs font-medium rounded ${statusColors[post.status] || 'bg-gray-100 text-gray-800'}`}
                          >
                            {post.status}
                          </span>
                        </td>
